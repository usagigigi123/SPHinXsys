...
BoundingBox system_domain_bounds(Vec3d(-0.5 * DH - BW, -BW, -0.5 * DH - BW),
	Vec3d(0.5 * DH + BW, DL + BW, 0.5 * DH + BW));
BoundingBox fluid_body_domain_bounds(Vec3d(-0.5 * DH, 0., -0.5 * DH),
	Vec3d(0.5 * DH, DL, 0.5 * DH));
Vec3d emitter_halfsize = Vec3d(0.5 * DH, 0.5 * BW, 0.5 * DH);
Vec3d emitter_translation = Vec3d(0.0, BW/2., 0.0);
...
Real U_f = 30.e-6/60./(Pi/4*DH*DH);
Real c_f = 10.0 * U_f;      	            /**< Speed of sound. */
Real mu_f = 6.5e-3;	                        /**< Approximate Dynamics viscosity of Blood. */
//Real mu_p = 4.e-4;
//Real lambda = 0.06;
//Real mu_0 = 1.6e-1;
//Real lambda = 8.2;
...
class WaterBlock : public ComplexShape
{
    public:
        ...
        {
            Real radius = DH/2.;
            Real halflength = DL/2.;
            Vecd translation_water(0.0, DL/2.0, 0.0);
            add<TriangleMeshShapeCylinder>(SimTK::UnitVec3(0, 1.0, 0), radius, halflength, resolution, translation_water);
        }
};
...
public:
    ...
    {
        Real radius = DH/2.;
        Real halflength = DL/2 + BW;
        Vecd translation_wall(0.0, DL/2., 0.0);
        add<TriangleMeshShapeCylinder>(SimTK::UnitVec3(0, 1.0, 0), radius + BW, halflength, resolution, translation_wall);
        subtract<TriangleMeshShapeCylinder>(SimTK::UnitVec3(0, 1.0, 0), radius, halflength, resolution, translation_wall);
    }
};


/** Case dependent inflow boundary condition. */
class UniformInflow : public fluid_dynamics::InflowBoundaryCondition
{
public:
	UniformInflow(FluidBody &body, BodyAlignedBoxByCell &aligned_box_part)
...
public:
	...
	Vecd getTargetVelocity(Vecd& position, Vecd& velocity)
	{
		...
		Real v = velocity[1];
        Real w = velocity[2];
        Real sponge_tip = DL_sponge + BW;
        //Real sponge_tip = DL_in + BW + DL_sponge;
		if (position[1] < halfsize_[1]) {
            Real a = sqrt(position[0]*position[0]+position[2]*position[2]);
			u = 0.0;
            //v = (-6.0 * a * a / DH / DH + 1.5) * u_ave_;
            //v = 1.5 * u_ave_ * (1 - a * a / halfsize_[0] / halfsize_[0]);
            v = 1.5 * U_f * (1 - a * a / halfsize_[0] / halfsize_[0]);
            w = 0.0;
		}
		return Vecd(u, v, w);
	}
	...
	void setupDynamics(Real dt = 0.0) override
	{
        ...
        //u_ave_ = u_ref_ * 0.5 * (1.0 + sin(Pi * run_time / t_ref_ - 0.5 * Pi));
...
public:
    ...
    {
        int ny = 11;
        int nz = number_of_particles + 1;
        for (int i = 0; i < ny; i++)
        {
            Real y = DL / (ny - 1) * i;
            for (int j = 0; j < nz; j++)
            {
                Real z = -DH/2. + DH / (nz - 1) * j;
                Vec3d point_coordinate(0.0, y, z);
                positions_.push_back(point_coordinate);
            }
        }
    }
...
int main()
{
    ...
    //fluid_dynamics::DensityRelaxationWithWallShearThinning density_relaxation(water_block_complex);
    /** Computing viscous acceleration. */
    fluid_dynamics::UpdateViscosity update_viscosity(water_body_inner, mu_0, lambda);
